...
import type { GameState, EnemyData, BulletData, EnemyBulletData } from "@/types/game";
import type { CharacterDef, WeaponDef, GameMode, Outfit } from "@/gameData";
import { WEAPONS, COSMETICS, GOLD_PER_KILL, GOLD_WIN_BONUS, GOLD_BR_WIN } from "@/gameData";
...
function makeEnemies(mode: GameMode, level: number): EnemyData[] {
  ...
  const pool  = mode==="battle-royale" ? MORE : BASE;
  const count = mode==="quick" ? Math.min(5+(level-1)*2,14) : mode==="battle-royale" ? 20 : 8;
  const spdMult = 1+(level-1)*0.12;
  return Array.from({length:count},(_,i)=>{
    const angle=Math.random()*Math.PI*2, dist=8+Math.random()*26, spd=(1.5+Math.random()*1.5)*spdMult, va=Math.random()*Math.PI*2;
    return { id:i, alive:true, color:pool[i%pool.length], position:[Math.cos(angle)*dist,0.75,Math.sin(angle)*dist] as [number,number,number], velocity:[Math.cos(va)*spd,Math.sin(va)*spd] as [number,number], changeTimer:Math.random()*3 };
  });
}
...
export default function Game({ character, loadout, gameMode, level, gold, diamonds, outfit, onGameEnd }: GameProps) {
  ...
  const resetGame=useCallback(()=>{
    ...
    setBullets([]); setEnemyBullets([]); setKills([]); setEarnedGold(0); setEarnedDia(0);
    setCollectibles(INIT_COLLECTIBLES); setZoneRadius(BR_ZONE_START);
    setShowFlash(false); setGlooWalls([]); setIsZoomed(false); setActiveSlot(0);
  ...
  const handleCreateGloo=useCallback(()=>{
    ...
    setGlooWalls((prev)=>[...prev,{id:Date.now(),position:wallPos,rotation:yaw,hp:300}]);
  },[glooWalls.length]);
  ...
  return (
    <div style={{ width:"100vw",height:"100vh",touchAction:"none",overflow:"hidden" }}>
      <Canvas
        shadows={!isTouch}
        camera={{ fov: isZoomed?20:70, near:0.1,far:500,position:[0,9,-8] }}
      ...
      >
        ...
        <KeyboardControls map={keyMap}>
          <GameScene
            gameStateRef={gameStateRef} enemies={enemies} bullets={bullets}
            enemyBullets={enemyBullets} collectibles={collectibles}
            inputRef={inputRef} isTouch={isTouch} hp={hp}
            character={character} weapon={activeWeapon} gameMode={gameMode}
          ...
          {/* Gloo walls in 3D */}
          {glooWalls.map((w)=>(
            <GlooWall key={w.id} wall={w} bullets={bullets} onDamage={handleGlooDmg} />
          ))}
        </KeyboardControls>
      </Canvas>
      ...
      {gameState==="start" && (
        <div className="overlay">
          ...
          <p className="subtitle">{isTouch ? "Joystick · Glisse pour viser · 🔥 TIRER · ⬆ SAUTER" : "ZQSD · Espace sauter · Clic G tirer · Clic D visée · G = Gloo Wall · 1-4 changer arme"}</p>
      ...
      {gameState==="gameover" && (
        <div className="overlay gameover-overlay">
          ...
          <div style={{ fontSize:22,color:"#fff" }}>Score : <span style={{ color:"#ffd700" }}>{score}</span> · 🪙 {earnedGold} {earnedDia>0&&`· 💎 ${earnedDia}`}</div>
      ...
      {gameState==="win" && (
        <div className="overlay win-overlay">
          ...
          <div style={{ fontSize:18,color:"#ffd700" }}>🪙 +{earnedGold+(gameMode==="battle-royale"?GOLD_BR_WIN:GOLD_WIN_BONUS)} or {earnedDia>0&&`· 💎 +${earnedDia} diamants`}</div>
          <div style={{ display:"flex",gap:14,flexWrap:"wrap",justifyContent:"center",marginTop:8 }}>
